...
from utils import *
from hvit_model import HVITModel

class HVIT(VAEMixin, UnsupervisedTrainingMixin, BaseModelClass):
    """
    ...
    **model_kwargs
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        super(HVIT, self).__init__(adata)
        self.manifold = manifold
        self.adata = adata
        self._is_linear_decoder = linear_decoder
        self.module = HVITModel(
            self.manifold,
    ...
    def predict(
        ...
        if not regression:
            ...
            latent_ctrl = torch.tensor(latent_ctrl)
            latent_treat = np.average(self.get_latent_representation(treat_x), axis = 0)
            latent_treat = torch.tensor(latent_treat)
            delta = self.manifold.dist(latent_treat, latent_ctrl).cpu().detach().numpy()
        else:
            ...
            for cell in cell_types:
                if cell != cell_type_to_predict:
                    ...
                    ctrl_centroid = torch.tensor(np.average(latent_ctrl.X, axis = 0))
                    treat_centroid = torch.tensor(np.average(latent_treat.X, axis = 0))
                    delta = self.manifold.dist(treat_centroid, ctrl_centroid).cpu().detach().numpy()
                    deltas.append(delta)
